...
pub(crate) type ChildIndex = bool;
...
impl NodePath {
    ...
    pub fn new_from_vec(num_bits: usize, bytes: Vec<u8>) -> Self {
        let bits = BitVec::try_from_vec(bytes).unwrap();
        assert!(
    ...
    pub fn push(&self, child_index: ChildIndex) {
...
impl InternalNode {
    ...
    pub fn serialize(&self, binary: &mut Vec<u8>) -> Result<()> {
        ...
        for _ in 0..existence_bitmap.count_ones() {
            let next_child = existence_bitmap.trailing_zeros();
